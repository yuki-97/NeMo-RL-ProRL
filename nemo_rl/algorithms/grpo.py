...
def setup(
    ...
    use_env_dapo = master_config["env"].get("use_dapo", False)
    dataloader = StatefulDataLoader(
        dataset,
        batch_size=1 if use_env_dapo else grpo_config["num_prompts_per_step"],
...
def grpo_train(
    ...
    def run_step(
        batch: BatchedDataDict[DatumSpec] | None = None, use_env_dapo: bool = False
    ) -> None:
        nonlocal current_step, total_steps, current_epoch
        nonlocal consumed_samples, total_valid_tokens
        ...
        with timer.time("total_step_time"):
                ...
                if not master_config["data"].get("use_raw_data", False):
                    with timer.time("data_processing"):
                        ...
                        repeated_batch: BatchedDataDict[DatumSpec] = (
                            batch.repeat_interleave(samples_per_prompt_group)
                        )
            ...
            if use_env_dapo:
                repeated_batch_size = requested_batch_size * samples_per_prompt_group
            else:
                repeated_batch_size = repeated_batch.size
            print(
                f"▶ Generating responses for batch of size {repeated_batch_size}...",
                flush=True,
            )
        ...
        number_of_samples_per_step = (
            master_config["grpo"]["num_prompts_per_step"] * samples_per_prompt_group
        )
        total_num_gpus = (
        ...
        return should_save_by_timeout

    while current_epoch < max_num_epochs and total_steps < max_num_steps:
        if use_env_dapo:
            # data stream will be handled by the environment
            should_save_by_timeout = run_step(None, use_env_dapo=True)
            if should_save_by_timeout or total_steps >= max_num_steps:
                return
        else:
            print(f"\n{'=' * 25} Epoch {current_epoch + 1}/{max_num_epochs} {'=' * 25}")
            for batch in dataloader:
                should_save_by_timeout = run_step(batch, use_env_dapo=False)
                if should_save_by_timeout or total_steps >= max_num_steps:
                    return
            current_epoch += 1
...
def validate(
    ...
    with timer.time("total_validation_time"):
        ...
        for batch_idx, val_batch in enumerate(val_dataloader):
            ...
            # Run rollouts in the environment
            if _should_use_rollout_in_env(master_config):
                val_batch, gen_metrics = val_task_to_env.run_async_rollout(
                    val_batch, is_val=True
                )
            # Use async rollouts if vLLM async engine is enabled
            elif _should_use_async_rollouts(master_config):
        ...
        accuracy = {
            ds: sum(total_rewards[ds]) / len(total_rewards[ds]) for ds in total_rewards
        }

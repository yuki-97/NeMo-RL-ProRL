...
class OpenhandsEnvironmentDAPO(OpenhandsEnvironment):
    ...
    def __init__(self, config: MasterConfig, tokenizer, server_addresses: List[str], dp_size: int):
    ...
    def run_async_rollout_dapo(self, requested_batch_size: int) -> tuple[BatchedDataDict, dict]:
        ...
        output_messages, output_batch = asyncio.run(self.request_from_openhands_dapo(requested_batch_size))
        request_end_time = time.time()
        
        logger.info(f"request_from_openhands_dapo time: {request_end_time - request_start_time:.3f}s")
        ...
        logger.info(f"Results2BatchedDataDict time: {convert_results_end_time - convert_results_start_time:.3f}s")
    ...
    async def refill_job_queue(self, data_index: int) -> tuple[BatchedDataDict, int]:
        ...
        """
        assert self.data_loader is not None, "data_loader must be set before calling refill_job_queue"
        ...
        try:
                                    ...
                                    if self.all_input_batch is None:
                                        ...
                                        for key in self.all_input_batch.keys():
                                            if isinstance(self.all_input_batch[key], torch.Tensor):
                                                ...
                                                combined_dict[key] = self.all_input_batch[key] + input_batch[key]
                                        self.all_input_batch = BatchedDataDict(combined_dict)
                                    continue
            ...
            while True:
                ...
                if num_completed_instances >= requested_batch_size:
                    logger.info(f"Completed {num_completed_instances} instances, requested batch size is {requested_batch_size}")
                    break
            ...
            instance_ids_in_output_batch = set([
                instance['instance_id'] 
                for instance in output_batch['instance']
            ])
            ...
            assert instance_ids_before_filtering == (
                instance_ids_after_filtering | instance_ids_in_output_batch | instance_ids_in_filtered_instance_ids
            ), (
    ...
    async def push_remaining_train_data_to_job_queue(self):
        ...
        for message in messages:
            await self.job_queue.put((
                ...
                (self.last_data_index, message, 0)
            ))
    ...
    def filter_uncompleted_instances(
        ...
        for key in self.all_input_batch.keys():
            if isinstance(self.all_input_batch[key], torch.Tensor):
                ...
                output_dict[key] = [self.all_input_batch[key][i] for i in batch_idx_to_keep]
        ...
        for i, instance in enumerate(self.all_input_batch['instance']):
            instance_id = instance['instance_id']
            if instance_id not in filtered_instance_ids and instance_id not in instance_ids_to_keep:
        ...
        for key in self.all_input_batch.keys():
            if isinstance(self.all_input_batch[key], torch.Tensor):
                ...
                remain_dict[key] = [self.all_input_batch[key][i] for i in batch_idx_to_remain]
